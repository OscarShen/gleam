...
public:
	...
	// vector noise
	glm::vec3 noise3f(glm::vec3 p)
	{
		return glm::vec3(noise(p),
			noise(p + glm::vec3(32, 78, 7)),
			noise(p + glm::vec3(123, 11, 96))
		);
	}

	// fractal sum
	float fBm(glm::vec3 p, int octaves = 4, float lacunarity = 2.0, float gain = 0.5)
	{
	...
	glm::vec3 fBm3f(glm::vec3 p, int octaves = 4, float lacunarity = 2.0, float gain = 0.5)
	{
		float freq = 1.0, amp = 0.5;
		glm::vec3 sum(0);
		for (int i = 0; i<octaves; i++) {
	...
	float turbulence(glm::vec3 p, int octaves = 4, float lacunarity = 2.0, float gain = 0.5)
	{
