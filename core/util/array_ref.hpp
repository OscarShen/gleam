...
#ifndef GLEAM_ARRAYREF_HPP
#define GLEAM_ARRAYREF_HPP
...
namespace gleam
{
	...
	public:
		...
		{
			CHECK_INFO(n <= this->size(), "Invalid specifier");
			return ArrayRef<T>(this->data() + n, this->size() - n);
		}

		ArrayRef<T> Slice(uint32_t n, uint32_t m) const
		{
			CHECK_INFO(n + m <= this->size(), "Invalid specifier");
			return ArrayRef<T>(this->data() + n, m);
		}

		ArrayRef<T> DropBack(uint32_t n = 1) const
		{
			CHECK_INFO(this->size() >= n, "Dropping more elements than exist");
			return this->Slice(0, this->Size() - n);
		}

		T const & operator[](size_t index) const
		{
			CHECK_INFO(index < size_, "Invalid index!");
			return data_[index];
		}
